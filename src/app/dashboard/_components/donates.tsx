...
export function DonationTable() {
  ...
  const {data, isLoading} = useQuery({
    ...
    queryFn: async () => {
      ...
      const json = await response.json() as ResponeseData;
          ...
          <TableBody>
            {data && data.map((donation) => (
              <TableRow key={donation.id}>
                ...
                <TableCell className="max-w-72">{donation.donorMesage}</TableCell>
      ...
      <div className="lg:hidden space-y-4">
        {data && data.map((donation) => (
          <Card key={donation.id}>
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground mb-2">{donation.donorMesage}</p>
