...
export default async function Dashboard() {
  ...
  const urlStripeDashboard = await getStripeDashboard(session.user?.conectedStripeAccountId)
    ...
    <div className="p-4">
      <section className="flex items-center justify-between mb-4">
        <div className="w-full flex items-center gap-2 justify-between">
          ...
          {urlStripeDashboard && (
            <a href={urlStripeDashboard} className="bg-zinc-900 px-4 py-1 rounded-md text-white cursor-pointer" target="_blank">
            Ajustar conta
          </a>
        )}
      ...
      <Stats userId={session.user.id} stripeAccountId={session.user.conectedStripeAccountId ?? ""} />
      


      <h2 className="text-2xl font-semibold mb-2">Últimas doações</h2>
      {session.user.conectedStripeAccountId && <DonationTable />}
    </div>
  );
}